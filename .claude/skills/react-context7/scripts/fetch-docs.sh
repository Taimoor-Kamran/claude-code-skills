...
# React Context7 Documentation Fetcher
# Fetches and filters React documentation with token efficiency
...
Fetch React documentation from Context7 with token-efficient filtering.

Arguments:
    topic               The React topic to search for
...
Examples:
    $(basename "$0") "hooks"
    $(basename "$0") "useEffect" --section="cleanup"
    $(basename "$0") "state management" --verbose

Topics:
    hooks, components, state-management, performance, forms, effects,
    refs, error-boundaries, suspense, server-components, context,
...
filter_docs() {
    ...
    if echo "$raw_docs" | grep -q '"error"'; then
        ...
        case "$NORMALIZED_TOPIC" in
            *hook*|usestate|useeffect|usecontext|usememo|usecallback|useref|usereducer)
                ref_file="${SCRIPT_DIR}/../references/hooks-reference.md"
                ;;
            *performance*|memo*|lazy|suspense)
                ref_file="${SCRIPT_DIR}/../references/performance-patterns.md"
                ;;
            *)
                ref_file="${SCRIPT_DIR}/../references/react-patterns.md"
                ;;
        esac
...
---
*Fetched from Context7 React documentation*
EOF
}
