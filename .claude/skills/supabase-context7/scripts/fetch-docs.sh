#!/bin/bash
# Main orchestrator: Token-efficient Supabase documentation fetcher
#
...
Token-efficient Supabase documentation fetcher using Context7 MCP

OPTIONS:
  --library-id ID    Context7 library ID (e.g., /supabase/supabase)
  --library NAME     Library name (will resolve to ID)
  --topic TOPIC      Topic to focus on (e.g., auth, storage, rls)
...
EXAMPLES:
  ...
  $0 --library-id /supabase/supabase --topic authentication

  # Resolve library name first
  $0 --library supabase --topic "row level security" --mode code

  # Get conceptual info
  $0 --library-id /supabase/supabase --topic "getting started" --mode info

  # Pagination
  $0 --library supabase --topic storage --page 2 --verbose
USAGE
  exit 1
}
