#!/bin/bash
# Main orchestrator: Token-efficient Tailwind CSS Animate documentation fetcher
#
...
Token-efficient Tailwind CSS Animate documentation fetcher using Context7 MCP

OPTIONS:
  --library-id ID    Context7 library ID (e.g., /jamiebuilds/tailwindcss-animate)
  --library NAME     Library name (will resolve to ID)
  --topic TOPIC      Topic to focus on (e.g., entrance animations, fade, duration)
...
EXAMPLES:
  ...
  $0 --library-id /jamiebuilds/tailwindcss-animate --topic "entrance animations"

  # Resolve library name first
  $0 --library "tailwindcss-animate" --topic "fade in" --mode code

  # Get conceptual info
  $0 --library-id /jamiebuilds/tailwindcss-animate --topic "installation" --mode info

  # Pagination
  $0 --library "tailwindcss-animate" --topic "animations" --page 2 --verbose
USAGE
  exit 1
}
