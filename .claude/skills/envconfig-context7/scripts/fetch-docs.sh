#!/bin/bash
# Main orchestrator: Token-efficient Envconfig documentation fetcher
#
...
Token-efficient Envconfig documentation fetcher using Context7 MCP

OPTIONS:
  --library-id ID    Context7 library ID (e.g., /kelseyhightower/envconfig)
  --library NAME     Library name (will resolve to ID)
  --topic TOPIC      Topic to focus on (e.g., struct tags, default, required)
...
EXAMPLES:
  ...
  $0 --library-id /kelseyhightower/envconfig --topic "struct tags"

  # Resolve library name first
  $0 --library envconfig --topic "default values" --mode code

  # Get conceptual info
  $0 --library-id /kelseyhightower/envconfig --topic "getting started" --mode info

  # Pagination
  $0 --library envconfig --topic "custom decoder" --page 2 --verbose
USAGE
  exit 1
}
