#!/bin/bash
# Main orchestrator: Token-efficient Django REST Framework documentation fetcher
#
...
Token-efficient Django REST Framework documentation fetcher using Context7 MCP

OPTIONS:
  --library-id ID    Context7 library ID (e.g., /encode/django-rest-framework)
  --library NAME     Library name (will resolve to ID)
  --topic TOPIC      Topic to focus on (e.g., serializers, viewsets, permissions)
...
EXAMPLES:
  ...
  $0 --library-id /encode/django-rest-framework --topic serializers

  # Resolve library name first
  $0 --library "django rest framework" --topic viewsets --mode code

  # Get conceptual info
  $0 --library-id /encode/django-rest-framework --topic "getting started" --mode info

  # Pagination
  $0 --library "django rest framework" --topic authentication --page 2 --verbose
USAGE
  exit 1
}
